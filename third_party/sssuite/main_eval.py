...
parser.add_argument('--resume_epoch', type=int, default=40, help='Run test with epoch number of trained model.')
...
##return bool boudary mask 
def load_boundary(gt_image):
    gt_image = gt_image.astype(np.uint32)
    mask_all = []
    config = [
            [0,0,0,0],
            [0,0,142,1],    #car,truck,bus,carvan,trailer,train
            [220,20,60,2],  #person and rider
            [128,64,128,3], #road
        ]

    H,W,_ = gt_image.shape
    label = np.zeros((H,W),np.uint32)
    label = (gt_image[:,:,0]*256 + gt_image[:,:,1])*256 +gt_image[:,:,2]
    for r,g,b,cate in config:
        c = (r*256+g)*256+b
        mask = np.zeros((H,W),np.uint8) #blank mask
        mask[label==c] = 255
        ##sobel_mask
        ...
        dst = cv2.addWeighted(absX,1.0,absY,1.0,0)

        #dilate mask
        kernel = np.ones((30,30), np.uint8)
        dilation = cv2.dilate(dst,kernel,iterations=1)
        d_mask = np.zeros((H,W),np.uint8) # blank mask
        d_mask[dilation>0] = 1
        #d_mask = cv2.cvtColor(d_mask,cv2.COLOR_GRAY2RGB)
        mask_all.append(d_mask)
    return mask_all
...
model_checkpoint_name = os.path.join("checkpoints","%04d" % args.resume_epoch,"model.ckpt")
...
if args.mode == "train":
    ...
    # Create directories if needed
    if not os.path.isdir("%s/%04d"%("Val",args.resume_epoch)):
        os.makedirs("%s/%04d"%("Val",args.resume_epoch))

    target=open("%s/%04d/val_scores.csv"%("Val",args.resume_epoch),'w')
    target.write("val_name, avg_accuracy, precision, recall, f1 score, mean iou, %s\n" % (class_names_string))
    target1=open("%s/%04d/val_car_scores.csv"%("Val",args.resume_epoch),'w')
    target1.write("val_name, avg_accuracy, precision, recall, f1 score, mean iou, %s\n" % (class_names_string))
    target2=open("%s/%04d/val_person_scores.csv"%("Val",args.resume_epoch),'w')
    target2.write("val_name, avg_accuracy, precision, recall, f1 score, mean iou, %s\n" % (class_names_string))
    target3=open("%s/%04d/val_road.csv"%("Val",args.resume_epoch),'w')
    target3.write("val_name, avg_accuracy, precision, recall, f1 score, mean iou, %s\n" % (class_names_string))
    ##
    scores_list = []
    class_scores_list = []
    precision_list = []
    ...
    for ind in range(len(val_input_names)):
        ...
        input_image = np.expand_dims(np.float32(load_image(val_input_names[ind])[:args.crop_height, :args.crop_width]),axis=0)/255.0
        gt = load_image(val_output_names[ind])[:args.crop_height, :args.crop_width]
        _,gt1,gt2,gt3 = load_boundary(gt) #gt for each category
        gtt1 = cv2.cvtColor(gt1,cv2.COLOR_GRAY2RGB)
        gtt2 = cv2.cvtColor(gt2,cv2.COLOR_GRAY2RGB)
        gtt3 = cv2.cvtColor(gt3,cv2.COLOR_GRAY2RGB)
        ...
        accuracy, class_accuracies, prec, rec, f1, iou = utils.evaluate_segmentation(pred=output_image, label=gt, num_classes=num_classes)
        ##
        accuracy1, class_accuracies1, prec1, rec1, f1_1, iou1 = utils.evaluate_segmentation(pred=output_image*gt1, label=gt*gt1, num_classes=num_classes)
        accuracy2, class_accuracies2, prec2, rec2, f1_2, iou2 = utils.evaluate_segmentation(pred=output_image*gt2, label=gt*gt2, num_classes=num_classes)
        accuracy3, class_accuracies3, prec3, rec3, f1_3, iou3 = utils.evaluate_segmentation(pred=output_image*gt3, label=gt*gt3, num_classes=num_classes)
        ## 
        file_name = utils.filepath_to_name(val_input_names[ind])
        target.write("%s, %f, %f, %f, %f, %f"%(file_name, accuracy, prec, rec, f1, iou))
        target1.write("%s, %f, %f, %f, %f, %f"%(file_name, accuracy1, prec1, rec1, f1_1, iou1))
        target2.write("%s, %f, %f, %f, %f, %f"%(file_name, accuracy2, prec2, rec2, f1_2, iou2))
        target3.write("%s, %f, %f, %f, %f, %f"%(file_name, accuracy3, prec3, rec3, f1_3, iou3))
        for item in class_accuracies:
            target.write(", %f"%(item))
        ##
        for item in class_accuracies1:
            target1.write(", %f"%(item))
        for item in class_accuracies2:
            target2.write(", %f"%(item))
        for item in class_accuracies3:
            target3.write(", %f"%(item))
        ##
        target.write("\n")
        target1.write("\n")
        target2.write("\n")
        ...
        #cv2.imwrite("%s/%s_gt1.png"%("Val", file_name),cv2.cvtColor(np.uint8(gtt1*gt), cv2.COLOR_RGB2BGR))
        #cv2.imwrite("%s/%s_gt2.png"%("Val", file_name),cv2.cvtColor(np.uint8(gtt2*gt), cv2.COLOR_RGB2BGR))
        #cv2.imwrite("%s/%s_gt3.png"%("Val", file_name),cv2.cvtColor(np.uint8(gtt3*gt), cv2.COLOR_RGB2BGR))
