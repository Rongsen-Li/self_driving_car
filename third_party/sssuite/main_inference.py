...
if args.continue_training or not args.mode == "inference":
...
if args.mode == "train":
    ...
    print("")
    TEST_NUM = 100 
    target=open("%s/%s_inference.csv"%("Val",args.model),'w')
    target.write("Time\tParams\tMem_Max\tMem_InUse\n" % ())
    # to get the right aspect ratio of the output
    loaded_image = np.zeros([args.crop_height,args.crop_width,3],dtype=np.uint8)
    input_image = np.expand_dims(np.float32(loaded_image),axis=0)/255.0
    st = time.time()
    for i in range(TEST_NUM):
        # inference time test
        sess.run(network,feed_dict={net_input:input_image})
        pass
    #GPUtil.showUtilization()
    max_use = sess.run(tf.contrib.memory_stats.MaxBytesInUse())
    cur_use = sess.run(tf.contrib.memory_stats.BytesInUse())
    run_time = time.time()-st
    target.write("%.2f\t" % (run_time/TEST_NUM))
    num_params = 0
    for variable in tf.trainable_variables():
        shape = variable.get_shape()
        num_params += reduce(mul, [dim.value for dim in shape], 1)
    target.write("%s\t" % num_params )
    target.write("%s\t" % max_use)
    target.write("%s\n" % cur_use)
